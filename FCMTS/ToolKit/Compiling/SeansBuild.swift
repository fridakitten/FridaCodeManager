...
func build(_ ProjectInfo: Project,_ erase: Bool,_ status: Binding<String>?,_ progress: Binding<Double>?) -> Int {
    ...
    let bashenv: [String] = ["SDKROOT=\(info[3])","CPATH=\(Bundle.main.bundlePath)/include:\(jbroot)/usr/bin/include","LIBRARY_PATH=\(info[3])/usr/lib","FRAMEWORK_PATH=/System/Library/Frameworks:/System/Library/PrivateFrameworks"]
    ...
    if !SwiftFiles.isEmpty {
        EXEC += "echo \"swift is not supported on the TrollStore version yet because the toolchain doesn't have swift yet(very hard to implement)\"; exit 255"
    } else {
        EXEC += """
        clang-16 \(frameflags) \(apiextension.build) -target arm64-apple-ios\(ProjectInfo.TG) \(MFiles.joined(separator: " ")) \(AFiles.joined(separator: " ")) \
