...
func runCommand(_ command: String, _ args: [String], _ uid: uid_t,_ env: [String]) -> Int {
    ...
    let env = ["PATH=/usr/local/sbin:\(jbroot)/bin","HOME=/private/var/mobile","CURL_CA_BUNDLE=\(jbroot)/share/curl/cacert.pem","TMPDIR=\(global_documents)/../tmp","CURL_CA_BUNDLE=\(jbroot)/share/curl/cacert.pem"] + env
